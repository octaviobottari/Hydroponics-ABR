...
urltime = 'http://www.fechadehoy.com/argentina'
pagetime = requests.get(urltime)
souptime = BeautifulSoup(pagetime.content, 'html.parser')
ez = souptime.find_all('p', id ='fecha')
...
while(True):
    ...
    while True:
        ...
        mail = f"SELECT mail FROM systemsmails WHERE username='{userid}' ORDER BY id DESC LIMIT 1"
        ...
        for i in ez:            
            timeclient.append(i.text)
        timenuevo = timeclient[0]
        tiemponuevo = str(timenuevo).replace('[]', "")
        ...
        sql = f"INSERT INTO systems (nivelph, caudal, temperatura, tempagua, lugar, username, password, capacidad, systype, fecha) VALUES({ph}, {caudal}, '{temperatura}', '{tempagua}', '{lugares}', '{userid}', '{password}', '{capacidad}', '{systype}', '{tiemponuevo}')"
        print(sql)
        cursor = connection.cursor()
        cursor.execute(sql)
        connection.commit()
